	...
	def checkWinner(self):
		...
		for row in range (0,6):
			for start in range (0,4):
				if self.board[row][start]!=0:
					if self.board[row][start]==self.board[row][start+1]==self.board[row][start+2]==self.board[row][start+3]:
						return self.board[row][start]
		#Check for winner diagn - bottom-left to top-right
		for row in range (0,3):
			for start in range (0,4):
				if self.board[row][start]!=0:
					if self.board[row][start]==self.board[row+1][start+1]==self.board[row+2][start+2]==self.board[row+3][start+3]:
						return self.board[row][start]
		...
		for row in range (0,3):
			for start in range (0,7):
				if self.board[row][start]!=0:
					if self.board[row][start]==self.board[row+1][start]==self.board[row+2][start]==self.board[row+3][start]:
						return self.board[row][start]		
...
class GUI:
	...
	def drawBoard(self,board):
		...
		for i in range(0, 6):
			for j in range(0, 7):
				if board[i][j]==0:
					self.screen.blit(self.grid_tile,((j+1)*self.margin_left, (i+1)*self.margin_top))
				elif board[i][j]==1:
					self.screen.blit(self.red_token,((j+1)*self.margin_left, (i+1)*self.margin_top))
				elif board[i][j]==2:
					self.screen.blit(self.black_token,((j+1)*self.margin_left, (i+1)*self.margin_top))
